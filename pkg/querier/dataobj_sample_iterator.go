...
// range-vector evaluator. It applies the LogQL sample extractor to each line and emits one sample per
// extracted value.
//
...
func (it *dataObjSampleIterator) extract(rec dataObjLogRecord) []outSample {
	extractors := it.streamExtractorsFor(rec.fingerprint, rec.streamLabels)
	out := it.currPending[:0] // reuse: the previous batch is fully consumed
	for _, se := range extractors {
		samples, ok := se.Process(rec.timestamp, rec.line, rec.metadata)
		if !ok {
			continue
		}
		for _, es := range samples {
			out = append(out, outSample{
...
func (it *dataObjSampleIterator) streamExtractorsFor(fp uint64, streamLabels labels.Labels) []logqllog.StreamSampleExtractor {
	if se, ok := it.streamExtractors[fp]; ok {
		return se
	}
	lbls := labels.NewBuilder(streamLabels).Del(model.MetricNameLabel).Labels()
	se := make([]logqllog.StreamSampleExtractor, len(it.extractors))
	for i, e := range it.extractors {
		se[i] = e.ForStream(lbls)
	}
