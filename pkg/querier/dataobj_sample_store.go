...
func NewDataObjSampleStore(chunkStore Store, bucket objstore.Bucket, ms metastore.Metastore, logger log.Logger) Store {
...
func (s *dataObjSampleStore) SelectSamples(ctx context.Context, req logql.SelectSampleParams) (iter.SampleIterator, error) {
	...
	for i := range extractors {
		extractors[i], err = deletion.SetupExtractor(req, extractors[i])
		if err != nil {
			return nil, err
		}
	}
