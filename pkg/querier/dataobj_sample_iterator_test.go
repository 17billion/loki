...
func mustExtractors(t *testing.T, queries ...string) []syntax.SampleExtractor {
	t.Helper()
	var out []syntax.SampleExtractor
...
func TestDataObjSampleIterator_Next(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"no records": {
			queries: []string{`count_over_time({app="x"}[1m])`},
			records: nil,
			want:    nil,
		},
		...
		"multiple streams interleaved": {
			queries: []string{`count_over_time({app=~".+"}[1m])`},
			records: []dataObjLogRecord{
				testLogRecord(1, streamX, 100, "a"),
		...
		"multiple extractors emit several samples per record": {
			queries: []string{`count_over_time({app="x"}[1m])`, `bytes_over_time({app="x"}[1m])`},
			records: []dataObjLogRecord{testLogRecord(1, streamX, 100, "hello")},
			want: []emittedSample{
				{streamHash: 1, timestamp: 100, labels: streamX.String(), value: 1}, // count
				{streamHash: 1, timestamp: 100, labels: streamX.String(), value: 5}, // bytes: len("hello")
			},
		},
		"record dropped by line filter is skipped": {
			queries: []string{`count_over_time({app="x"} |= "keep" [1m])`},
			records: []dataObjLogRecord{
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			it := newDataObjSampleIterator(newTestLogReader(tc.records, nil), mustExtractors(t, tc.queries...))
...
func TestDataObjSampleIterator_At(t *testing.T) {
	...
	t.Run("zero value before Next", func(t *testing.T) {
		it := newDataObjSampleIterator(newTestLogReader(nil, nil), mustExtractors(t, `count_over_time({app="x"}[1m])`))
	...
	t.Run("count sample carries record timestamp and unit value", func(t *testing.T) {
		it := newDataObjSampleIterator(
			...
			mustExtractors(t, `count_over_time({app="x"}[1m])`),
		)
	...
	t.Run("bytes sample value is the line length", func(t *testing.T) {
		it := newDataObjSampleIterator(
			...
			mustExtractors(t, `bytes_over_time({app="x"}[1m])`),
		)
		require.True(t, it.Next())
...
func TestDataObjSampleIterator_Labels(t *testing.T) {
	...
	t.Run("renders the current record stream labels", func(t *testing.T) {
		...
		it := newDataObjSampleIterator(newTestLogReader(records, nil), mustExtractors(t, `count_over_time({app=~".+"}[1m])`))
...
func TestDataObjSampleIterator_StreamHash(t *testing.T) {
	...
	t.Run("tracks the current record fingerprint", func(t *testing.T) {
		...
		it := newDataObjSampleIterator(newTestLogReader(records, nil), mustExtractors(t, `count_over_time({app="x"}[1m])`))
...
func TestDataObjSampleIterator_Close(t *testing.T) {
	...
	t.Run("propagates the reader error", func(t *testing.T) {
		...
		it := newDataObjSampleIterator(newTestLogReader(nil, wantErr), mustExtractors(t, `count_over_time({app="x"}[1m])`))
