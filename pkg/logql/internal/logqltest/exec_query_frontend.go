...
func newQueryFrontendStack(t *testing.T, queryShardingEnabled bool) (*queryFrontendExecutionStack, error) {
	...
	if queryShardingEnabled {
		stackName = "query-frontend + query-scheduler (sharding)"
	} else {
		stackName = "query-frontend + query-scheduler (no sharding)"
	}
...
func (*queryFrontendExecutionStack) isEvalSupported(cmd evalCmd, exp expectations) bool {
